...
import { Appbar, useTheme, Button, Text, Divider, IconButton, Avatar, List, Portal, Dialog, TextInput, RadioButton } from "react-native-paper";
...
export default function Profil() {
                ...
                {index !== null ? (
                    <Dialog visible={visibleLimite} onDismiss={hideDialog}>
                        ...
                        <Dialog.Title style={styles.title}>
                            Modifier la limite {type ? "maximale" : "minimale"} du pré-set {userPreSets[index].nom}
                        </Dialog.Title>
                        <Dialog.ScrollArea>
                            <ScrollView contentContainerStyle={{ paddingHorizontal: 24 }}>
                                <TextInput label="Limite" value={limite} onChangeText={(text) => setLimite(text)} style={{ marginTop: 10 }} mode="outlined" theme={{ colors: { primary: theme.colors.primary } }} />
                            </ScrollView>
                        </Dialog.ScrollArea>
                        <Dialog.Actions>
    ...
    function EditPreset({ index }) {
        ...
        const choixIcone = ["walk", "train", "text-box-check", "clock", "bus", "star", "music-note", "tree"];
        return (
            <Portal>
                {index !== null ? (
                    <Dialog visible={visibleEditPreset} onDismiss={hideDialog}>
                        <Dialog.Icon icon="pencil" />
                        <Dialog.Title style={styles.title}>Modifier {userPreSets[index].nom}</Dialog.Title>
                        <Dialog.ScrollArea>
                            <ScrollView contentContainerStyle={{ paddingHorizontal: 24 }}>
                                <TextInput label="Nom" value={nom} onChangeText={(text) => setNom(text)} style={{ marginTop: 10 }} mode="outlined" theme={{ colors: { primary: theme.colors.primary } }} />
        ...
        <View style={styles.screen}>
            ...
            <ScreenWrapper>
                ...
                <List.Section style={styles.liste} title="Mes informations">
                    <List.Item title="Email" left={() => <List.Icon icon="email" style={{ marginLeft: 15 }} />} right={() => <Text>{email}</Text>} />
                    <List.Item title="Téléphone" left={() => <List.Icon icon="phone" style={{ marginLeft: 15 }} />} right={() => <Text>{telephone}</Text>} />
                    <List.Item
                        title="Mot de passe"
                        left={() => <List.Icon icon="lock" style={{ marginLeft: 15 }} />}
                        right={() => (
                            <>
                                <Text style={{ marginRight: 3, marginTop: 2 }}>{isHidden ? passwordHidden : password}</Text>
                            ...
                            return (
                                <View key={index}>
                                    <List.Accordion left={(props) => <List.Icon {...props} icon={item.icone} style={{ marginLeft: 7 }} />} title={item.nom}>
                                        ...
                                        />
                                        <List.Item
                                            left={(props) => <List.Icon {...props} icon="volume-plus" style={{ marginLeft: 15 }} />}
                                        ...
                                        />
                                        <List.Item
                                            left={(props) => <List.Icon {...props} icon="pencil" style={{ marginLeft: 15 }} />}
                                            title={"Renommer"}
                                            onPress={() => {
                                                setIndexDialog(index);
